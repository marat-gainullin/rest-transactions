...
public class TransactionsControllerCycleTest extends AbstractJUnit4SpringContextTests {
    ...
    public void cycleTest() throws Exception {
        mockMvc.perform(put("/transaction/{transaction-id}", 1)
                ...
                .content("{ \"amount\": 7000, \"type\": \"fruit\", \"parent_id\": 2 }")
        ).andExpect(status().isOk());
        mockMvc.perform(put("/transaction/{transaction-id}", 2)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{ \"amount\": 2000, \"type\": \"fruit\", \"parent_id\": 3}")
        ).andExpect(status().isOk());
        mockMvc.perform(put("/transaction/{transaction-id}", 3)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{ \"amount\": 3000, \"type\": \"fruit\", \"parent_id\": 1} }")
        ).andExpect(status().isOk());
        mockMvc.perform(get("/sum/{transaction-id}", 1))
                .andExpect(status().isConflict())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(content().json("{\"status\": \"Cycle in transaction tree with transaction id " + 1 + " detected\"}"));
    }
}
